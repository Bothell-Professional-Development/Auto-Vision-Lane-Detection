...
double bezier_calc(float wheelAngle, cv::Point2f &PointZero, cv::Point2f &PointOne, cv::Point2f &PointTwo, cv::Point2f &PointThree, double OutputArray[steerCommandArraySize])
{
	...
	PointTwo = { BezierMagnitude * sin(atan((PointTwo.x - PointThree.x) / (PointTwo.y - PointThree.y + std::numeric_limits<float>::min()))) ,
		BezierMagnitude * cos(atan((PointTwo.x - PointThree.x) / (PointTwo.y - PointThree.y + std::numeric_limits<float>::min()))) };
