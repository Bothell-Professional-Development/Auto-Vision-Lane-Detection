...
double bezier_calc(float wheelAngle, cv::Point2f &PointZero, cv::Point2f &PointOne, cv::Point2f &PointTwo, cv::Point2f &PointThree, double OutputArray[steerCommandArraySize])
{
	...
	PointTwo = { BezierMagnitude * sin(atan((PointThree.x - PointTwo.x) / (PointThree.y - PointTwo.y + std::numeric_limits<float>::min()))) , BezierMagnitude * cos(atan((PointThree.x - PointTwo.x) / (PointThree.y - PointTwo.y + std::numeric_limits<float>::min()))) };
